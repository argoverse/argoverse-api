...
def save_bev_img(path_output_vis, list_bboxes, list_difficulty_att, dataset_name, log_id, lidar_timestamp, pc, egovehicle_to_city_se3):
    """
    ...
    for bbox,difficulty_att  in zip(list_bboxes, list_difficulty_att):
        ...
        theta_local = np.arctan2(2*(q0*q3+q1*q2), 1-2*(q2*q2+q3*q3))#np.arccos(bbox['rotation']['w'])*2
    ...
    for ind_log, path_log in enumerate(list_log_folders): 
        ...
        if check_track_label_folder: 
            ...
            for key in dist_track_labels.keys():
                data_amodel[key] = {}
                data_amodel[key]['label_class'] = dist_track_labels[key][0]['label_class']
                data_amodel[key]['uuid'] = dist_track_labels[key][0]['track_label_uuid']
        ...
        for id_track in data_amodel.keys():
            ...
            dict_tracks[id_track]["list_vel"], dict_tracks[id_track]["list_acc"] = compute_v_a(dict_tracks[id_track]["list_center_w"])
            dict_tracks[id_track]["num_missing"] = dict_tracks[id_track]["length_track"]-dict_tracks[id_track]["exists"].sum()            
            ...
            if dict_tracks[id_track]["length_track"] == 0:
                ...
                if dict_tracks[id_track]["list_dist"][ind_valid].min() > dist_close:
                    ...
                    if dict_tracks[id_track]["length_track"] < 10 or dict_tracks[id_track]["exists"].sum() < 10:
                        ...
                        if (ind_close_max-ind_close_min) - dict_tracks[id_track]["exists"][ind_close_min:ind_close_max].sum() > 20:
        ...
        if visualize:
            for ind_lidar, timestamp_lidar in enumerate(list_lidar_timestamp): 
                ...
                list_lidar_timestamp = data_log.lidar_timestamp_list
                save_bev_img(path_output_vis, list_bboxes,list_difficulty_att, "argoverse_%s" % name_folder, id_log, timestamp_lidar, pc, egovehicle_to_city_se3)
