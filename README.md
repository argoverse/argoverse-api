...
`p_dst = dst_SE3_src * p_src` or `p_dst = dst_SE2_src * p_src`. We'll describe them in more detail below:

- **Map pose**: localization of the ego-vehicle inside the city (i.e. map) reference frame, `city_SE3_egovehicle`. Provided for each log in `poses/city_SE3_egovehicle_{nanosec_timestamp}.json`. This brings a point in the egovehicle's reference frame into the city's reference frame.
- **Camera extrinsics**: for each camera sensor, we provide in `vehicle_calibration_info.json` `egovehicle_SE3_camera`, and when we form the extrinsics matrix, we use its inverse, `camera_SE3_egovehicle`, i.e. bringing points into the camera coordinate frame before perspective projection.
- **Object labels**: a `egovehicle_SE3_object` transformation is provided for each annotated cuboid, that takes it from the labeled object's frame (at its center), to the egovehicle's reference frame. This data is provided per LiDAR sweep at `per_sweep_annotations_amodal/tracked_object_labels_{nanosec_timestamp}.json`.
...
A simple example for understanding the *object labels* -- imagine the ego-vehicle is stopped at a 4-way intersection at a red light, and an object is going straight through the intersection w/ green light, moving from left to right in front of us. If that labeled object is instantaneously 4 meters ahead of the egovehicle (+4 m along the x-axis), then the annotation would be the rotation to align the labeled object with the egovehicle's x-axis. Since we use the right-hand rule, we need 90 degree rotation about the "z"-axis to align the object's x-axis with the egovehicle's x-axis. If (0,0,0) is the origin of the labeled object, to move itself into the egovehicle frame, you would need to add 4 to its x coordinate, thus adding the translation vector (4,0,0). 
